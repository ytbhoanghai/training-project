...
export class RoleManagementService {
  ...
  findAllRoles(): Observable<IRole[]> {
    if (location.pathname.startsWith('/admin'))
      return this.httpClient.get<IRole[]>(this.ADMIN_URL + '/roles').pipe(
        catchError((err) => {
          console.error(err);
          return of([]);
        })
  ...
  findAllAdminRoles(): Observable<IRole[]> {
    return this.httpClient.get<IRole[]>(this.ADMIN_URL + '/roles').pipe(
      catchError((err) => {
        console.error(err);
        return of([]);
  ...
  deleteAdminRoleById(id: number): Observable<string> {
    return this.httpClient.delete<any>(this.ADMIN_URL + '/roles/' + id).pipe(
      tap((_) => console.log('delete role by id ' + id)),
      map((value) => value.message)
    );
  }
    ...
    return this.httpClient.get<IResource[]>(SERVER_URL + '/resources').pipe(
      map((resources) => {
        for (let i = 0; i < resources.length; i++) {
          resources[i].permissions = this.permissionListResolver(
  ...
  createAdminRole(body: IRoleBody): Observable<IRole> {
    return this.httpClient.post<IRole>(this.ADMIN_URL + '/roles', body);
  }

  updateRole(id: number, body: IRoleBody): Observable<IRole> {
    if (location.pathname.startsWith('/admin'))
      return this.httpClient.put<IRole>(this.ADMIN_URL + `/roles/${id}`, body);
    // else if (location.pathname.startsWith('/my-store'))
    //   return this.httpClient.put<IRole>(this.MANAGER_URL + `/roles/${id}`, body);
    else
      return this.httpClient.put<IRole>(SERVER_URL + `/roles/${id}`, body);
  }

  updateAdminRole(id: number, body: IRoleBody): Observable<IRole> {
    return this.httpClient.put<IRole>(SERVER_URL + `/roles/${id}`, body);
  }

  private permissionListResolver(
